...
import API from '../api/axios';
import {
    Search, LogOut, Plus, Trash2, CheckCircle,
    Calendar, List, ChevronDown, MoreVertical, Edit,
    Clock, AlertCircle, User, X, Menu
} from 'lucide-react';
import { format } from 'date-fns';
import CalendarView from '../components/CalendarView';
...
const Dashboard = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50">
                ...
                <div className="max-w-7xl mx-auto px-4 py-3">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-gray-500 hidden md:inline">Dashboard</span>
                        </div>
            ...
            {/* Mobile Sidebar */}
            <div className={`
                fixed top-0 left-0 h-full w-64 bg-white shadow-xl z-40 md:hidden
                transform transition-transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}
            `}>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            <div className="relative flex-1">
                                ...
                                <input
                                    type="text"
                                    placeholder="Search tasks..."
                                    className="w-full pl-10 pr-4 py-2 border rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                                    value={searchTerm}
                                    onChange={(e) => setSearchTerm(e.target.value)}
                                />
                            </div>
                    ...
                    <div className="p-4 border-b">
                        <form onSubmit={addTask} className="space-y-3">
                            ...
                            <div className="flex flex-col sm:flex-row gap-3">
                                ...
                                <select
                                    className="px-4 py-2 border rounded bg-white outline-none"
                                    value={newTask.status}
                                    onChange={(e) => setNewTask({ ...newTask, status: e.target.value })}
                                    disabled={isAddingTask}
                                >
                                                ...
                                                <div className="flex items-start gap-3 flex-1">
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <div className="flex flex-wrap items-center gap-2 mt-2">
                                                            <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs ${getStatusColor(task.status)}`}>
                                                                {getStatusIcon(task.status)}
                                                                <span>{task.status}</span>
                                                            </span>
                                                            {task.dueDate && (
